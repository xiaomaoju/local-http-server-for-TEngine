...
- **两层版本模型** — 引入"项目版本"层（v1/v2/...），URL 形式 `/res/{项目版本}/{项目}/{平台}/{文件}`，可同时对外提供多套版本（适合 SDK 灰度 / 多渠道）
- **平台访问开关** — 每个（项目版本 × 平台）独立开关，关闭后该路径的资源访问立即返回 403，无需重启
- **激活逻辑下沉为元数据** — 不再复制文件到平台根目录，激活只是切换"当前 bundle"标记，省一倍磁盘
- **项目版本重命名** — 双击 L2 标签页 / 项目设置内联编辑均可改名，磁盘目录同步重命名
- **删除二次确认** — 项目 / 项目版本 / Bundle 三级删除均走原生确认弹窗
- **日志增强** — 每条日志多一列 scope chip 显示 `项目/版本`；平台开关切换、版本重命名等操作均广播日志
- **WebSocket 日志走 Tauri 原生** — 客户端 ws 连接迁移到 `tauri-plugin-websocket`，避免 webview CORS 问题
- **版本号集中管理** — `npm version X` 一键同步 `package.json` / Cargo.toml × 2 / tauri.conf.json / App.vue 显示文本
